...
# to use the BCV distance from edgeR:
# we insert a line of code to return the 
# distance matrix before the plot() is called
noplotMDS <- plotMDS.DGEList
body(noplotMDS)[[22]] <- quote(return(dd))
...
m <- 20
k <- 4
methods <- c("norm Eucl","log2 Eucl","rlog Eucl","VST Eucl","PoisDist","edgeR BCV")
  ...
  do.call(rbind, lapply(rnormsds[[idx]], function(rnormsd) {
    do.call(rbind, lapply(seq_along(sfs), function(sf.idx) {
      ...
      do.call(rbind, lapply(seq_len(nreps), function(i) {
        ...
        edgerBCV <- as.dist(noplotMDS(calcNormFactors(DGEList(mat)), method="bcv"))
        ...
        edgerARI <- adjustedRandIndex(condition, cutree(hclust(edgerBCV),k=k))

        data.frame(ARI = c(normARI, lognormARI, rlogARI, vstARI, poiDistARI, edgerARI),
                   method = methods,
...
save(res, file="../../data/results_simulateCluster.RData")
