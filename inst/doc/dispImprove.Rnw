...
%\VignetteIndexEntry{dispImprove}

\documentclass{article}
\usepackage[margin=2cm]{geometry}
\title{Assessing dispersion estimation and shrinkage}
\author{Michael Love}
\begin{document}
\maketitle

\section{Estimates of dispersion in DESeq2}

We constructed a simulated dataset with 10 samples divided into 2
