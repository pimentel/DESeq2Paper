...
%\VignetteIndexEntry{posterior}

\documentclass{article}
\usepackage[margin=2cm]{geometry}
\title{Shrinkage of logarithmic fold changes}
\author{Michael Love}
\begin{document}
\maketitle

\section{Comparing the posterior distribution for two genes}

First, we run a DE analysis on the Bottomly \textit{et al.} dataset, 
once with shrunken LFCs and once with unshrunken LFCs.
...
The following code, not run, was used to find two genes with similar average
expression strength and unshrunken LFC, but with disparate dispersion estimates
(and therefore Wald statistic in the unshrunken case).
...
We now extract the results from each run. Note that the second gene (with large dispersion)
has a high adjusted $p$-value with or without the shrinkage of LFCs.
